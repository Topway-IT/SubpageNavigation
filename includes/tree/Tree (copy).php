...
";
	...
	$cat = null;
		foreach ( $subpages as $t ) {
			$titlesText[] = $t->getText();
			$s = $this->renderNodeInfo( $t, $cat, $depth - 1, array_shift( $childrenCount ) );
		...
		if ( $inverse ) {
			...
			} elseif ( $mode !== TreeMode::ALL ) {
				if ( $mode === TreeMode::PAGES ) {
	 ...
	 * @return string
	 */
	public function renderNodeInfo( Title $title, Category $cat = null, $children = 0, $count = 0 ) {
		$mode = $this->getOption( 'mode' );
